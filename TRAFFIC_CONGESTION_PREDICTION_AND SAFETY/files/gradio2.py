...
#     original_image = cv2.imread(file.name)  # Read the image
#     original_image_color = cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB)  # Specify color for the image
...
#     result = reader.readtext(cropped_image)  # Reading English characters from the cropped number plate image

#     return result[0][1] if result else "No text detected."

# iface = gr.Interface(fn=process_image, inputs="file", outputs="text", live=True)
# iface.launch()
...
def process_image(file):
    ...
    return result[0][1] if result else "No text detected.", original_image_color

iface = gr.Interface(fn=process_image, inputs="file", outputs=["text", "image"], live=True)
iface.launch()
